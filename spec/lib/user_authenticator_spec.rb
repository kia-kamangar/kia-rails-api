...
describe UserAuthenticator do
  let(:user) { create :user, login: 'Kia KMG', password: 'secret' }
  ...
  context "when initialized with code" do
    ...
    describe "#initialize" do
      it "sould initialize proper authenticator" do
  ...
  context "when initialized with login & password" do
    let(:authenticator) { described_class.new(login: 'Kia KMG', password: 'secret') }
    let(:authenticator_class) { UserAuthenticator::Standard }

    describe "#initialize" do
      it "sould initialize proper authenticator" do
        expect(authenticator_class).to receive(:new).with('Kia KMG', 'secret')
        authenticator
      end
    end
